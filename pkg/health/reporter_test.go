...
func TestCountByStatus(t *testing.T) {
	...
	ok, warning, critical, unknown := countByStatus(results)
	assert.Equal(t, 2, ok)
...
func TestGroupByCategory(t *testing.T) {
	...
	assert.Len(t, grouped["Compute"], 1)
	assert.Len(t, grouped["Network"], 1)
}

func TestSeverityOrdering(t *testing.T) {
	assert.Less(t, severityRank(StatusCritical), severityRank(StatusWarning))
	assert.Less(t, severityRank(StatusWarning), severityRank(StatusUnknown))
	assert.Less(t, severityRank(StatusUnknown), severityRank(StatusOK))
}
...
func TestPrintCheckResultWithItems(t *testing.T) {
	...
	assert.Contains(t, output, "MonDistribution")
...
func TestPrintSummaryWithUnknown(t *testing.T) {
	results := []CheckResult{
		{Status: StatusOK},
		{Name: "TestCheck", Status: StatusUnknown, Message: "failed to get ceph status"},
	}
	...
	assert.Contains(t, output, "OK:           1")
	assert.NotContains(t, output, "Unknown:")
	assert.Contains(t, output, "Error:")
	assert.Contains(t, output, "could not determine status")
}

func TestPrintSummaryWithoutUnknown(t *testing.T) {
	results := []CheckResult{
		{Status: StatusOK},
		{Status: StatusWarning},
	}
...
func TestPrintReportFullOutput(t *testing.T) {
	results := []CheckResult{
		{Name: "CephClusterHealth", Category: "Storage", Status: StatusOK, Message: "HEALTH_OK"},
		{Name: "MonDistribution", Category: "Compute", Status: StatusWarning, Message: "2 mon pods on 2 nodes (at least 3 recommended)",
			Items: []CheckItem{
				{Name: "mon-a", Status: "Running", Node: "node1"},
				{Name: "mon-b", Status: "Running", Node: "node1"},
			}},
		{Name: "MGRStatus", Category: "Compute", Status: StatusOK, Message: "1 mgr pod(s) running"},
	}
	...
	monIdx := strings.Index(output, "MonDistribution")
	mgrIdx := strings.Index(output, "MGRStatus")
